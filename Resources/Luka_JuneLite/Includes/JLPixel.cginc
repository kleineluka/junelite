        ...
        switch (inputStyle) {
            ...
            case 3: // box
                ...
                [branch] if (inputSoften != 0) {
                    ...
                    // remap it from 0 to 1 to 0 to inputSoften
                    inputSoftenCalculations.x = saturate(doRemap(distanceToMidX, 0, 1.0, 0, inputSoften));
                    inputSoftenCalculations.y = saturate(doRemap(distanceToMidY, 0, 1.0, 0, inputSoften));
                    // combine the soften calculations
                    inputStatus.y = max(inputSoftenCalculations.x, inputSoftenCalculations.y);
                }
                break;
            default: // vertical
                // hard edge
...
void effectColoring(inout float4 inputColors,
    ...
    float inputGradingTone, float inputSharpness,
    float2 inputUVs, sampler2D inputPass,
    float inputRainbow, float inputRainbowSpeed) {
    // coloring effect 
    // note to self: no "universal toggle" for this module
    // sharpness
    [branch] if (inputSharpness != 0) {
        float2 texelSize = 1.0 / _ScreenParams.xy;
        float3 centerColor = tex2D(inputPass, inputUVs).rgb;
        float3 top = tex2D(inputPass, inputUVs + float2(0, texelSize.y)).rgb;
        float3 bottom = tex2D(inputPass, inputUVs - float2(0, texelSize.y)).rgb;
        float3 left = tex2D(inputPass, inputUVs - float2(texelSize.x, 0)).rgb;
        float3 right = tex2D(inputPass, inputUVs + float2(texelSize.x, 0)).rgb;
        float3 sharpened = centerColor * 5.0 - (top + bottom + left + right);
        inputColors.rgb = lerp(inputColors.rgb, sharpened, inputSharpness);
    }
    // rgb 
    ...
    // rainbow
    [branch] if (inputRainbow != 0) {
        float3 rainbowColors = makeRainbowFast(inputUVs, inputRainbowSpeed, 0, 0, 1, 1, 1);
        inputColors.rgb = lerp(inputColors.rgb, inputColors.rgb * rainbowColors.rgb, inputRainbow);
    }
    // invert
...
void effectBlur(inout float4 inputColors,
    ...
    float inputDithering) {
    // blur effect
    [branch] if (inputStyle != 0) {
        float4 cleanColors = inputColors;
        [branch] if (inputRadius != 1) inputPower *= makeRadialSmooth(1, inputRadius, inputUVs);
        float ditherValue = 0;
        [branch] if (inputDithering > 0)  ditherValue = getBayerValue(inputUVs * _ScreenParams.xy) * inputDithering;
        switch (inputStyle) {
            case 2: // radial
                ...
                [fastopt] for (int radialHQ = 0; radialHQ < 24; radialHQ++) {
                    ...
                    float2 thisUVs = lerp(inputUVs, inputZoomUVs, thisStepRadial * inputPower + ditherValue);
            ...
            case 3: // chromatic
                ...
                [fastopt] for (int chromaticHQ = 0; chromaticHQ < 16; chromaticHQ++) {
                    ...
                    thisColor.r = tex2D(inputPass, inputUVs + float2((inputPower * thisStep) + ditherValue, 0)).r;
                    thisColor.g = tex2D(inputPass, inputUVs).g;
                    thisColor.b = tex2D(inputPass, inputUVs - float2((inputPower * thisStep) + ditherValue, 0)).b;
                    inputColors.rgb += thisColor.rgb;
                }
                inputColors.rgb /= 16.0;
                #else 
                // low quality
                inputColors.r = tex2D(inputPass, inputUVs + float2(inputPower + ditherValue, 0)).r;
                inputColors.b = tex2D(inputPass, inputUVs + float2(-inputPower - ditherValue, 0)).b;
                #endif 
                break;
            default: // box
                inputPower *= 0.01;
                ...
                [fastopt] for (int iterBoxBasicX = -4; iterBoxBasicX < 4; iterBoxBasicX++) {
                    ...
                    [fastopt] for (int iterBoxBasicY = -4; iterBoxBasicY < 4; iterBoxBasicY++) {
                        ...
                        inputColors.rgb = blurKernelBoxFast(inputColors.rgb, inputPower + ditherValue, blurIterations, inputUVs, inputPass, inputColor);
                    }
                }
                inputColors /= 64.0;
                #else
...
void effectFilterTrippy(inout float4 inputColors,
    float inputStyle, float inputPower, 
    float inputSpread, float inputSpeed,
    float2 inputUVs) {
    // trippy effect
    [branch] if (inputStyle != 0) {
        float luma = getLuma(inputColors.rgb) * inputSpread;
        float3 rainbowEffect = makeRainbowFast(inputUVs, inputSpeed, 0, 0, luma, 1, 1);
        inputColors.rgb = lerp(inputColors.rgb, inputColors.rgb * rainbowEffect, inputPower);
    }
}
...
void effectOverlay(inout float4 inputColors,
    ...
    float inputAnimated, float inputFramesX,
    float inputFramesY, float inputFrames, 
    float inputSpeed, float inputScrub,
    float inputBlend, float inputVR,
    float inputVRPreview, float inputVRSizeX,
    float inputVRSizeY, float inputVROffsetX,
    float inputVROffsetY) {
    // overlay effect
    if (inputTransparency != 0) {
        float4 overlayControls = float4(inputSizeX, inputSizeY, inputOffsetX, inputOffsetY);
        [branch] if (inputVR != 0) {
            #if UNITY_SINGLE_PASS_STEREO
                overlayControls = float4(inputVRSizeX, inputVRSizeY, inputVROffsetX, inputVROffsetY);
            #else
                if (inputVRPreview) overlayControls = float4(inputVRSizeX, inputVRSizeY, inputVROffsetX, inputVROffsetY);
            #endif 
        }
        float2 overlayUVs = makeOverlayUVs(inputDirections, overlayControls);
        if (any(overlayUVs < 0 || overlayUVs > 1)) {
            inputTransparency = 0;
            return;
        }
        [branch] if (inputAnimated == 1) {
            overlayUVs = makeSpritesheet(inputFramesY, inputFramesX, inputFrames, inputSpeed, inputScrub, overlayUVs);
        }
        float4 overlayColor = tex2D(inputTexture, overlayUVs);
        [forcecase] switch (inputBlend) {
            case 1: // multiply
                inputColors.rgb = lerp(inputColors.rgb, inputColors.rgb * overlayColor.rgb, inputTransparency * overlayColor.a);
                break;
            case 2: // screen
                inputColors.rgb = lerp(inputColors.rgb, (1.0 - (1.0 - inputColors.rgb) * (1.0 - overlayColor.rgb)), inputTransparency * overlayColor.a);
                break;
            case 3: // add
                inputColors.rgb += (overlayColor.rgb * overlayColor.a);
                break;
            case 4: // subtract
                inputColors.rgb -= (overlayColor.rgb * overlayColor.a);
                break;
...
void effectGlitchUVs(inout float2 inputUVs,
    float inputScale, float inputAmount,
    float inputPower) {
    [branch] if (inputPower != 0) {
        float offset = 1.0 - makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 1, 20000);
        inputUVs.x += (offset * inputPower);
    }
}

void effectGlitchChromatic(inout float4 inputColors,
    float2 inputUVs, float inputScale,
    float inputAmount, float inputPower,
    sampler2D inputPass) {
    [branch] if (inputPower != 0) {
        float offsetR = 1.0 - makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 2.1, 20000);
        float offsetB = 1.0 - makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 4.5, 20000);
        float2 uvsR = inputUVs + float2(offsetR * inputPower, 0);
        float2 uvsB = inputUVs - float2(offsetR * inputPower, 0);
        float colorR = tex2D(inputPass, uvsR).r;
        float colorB = tex2D(inputPass, uvsB).b;
        inputColors.rgb = float3(colorR, inputColors.g, colorB);
    }
}
...
_LiteColoringSharpness, _LiteColoringRainbow, _LiteColoringRainbowSpeed;
#endif // _KEYWORD_ENABLE_LITE_COLORING
#ifdef _KEYWORD_ENABLE_LITE_BLUR
float _LiteBlurStyle, _LiteBlurRadius, _LiteBlurTransparency,
_LiteBlurPower, _LiteBlurDithering;
float4 _LiteBlurColor;
#endif // _KEYWORD_ENABLE_LITE_BLUR
...
_LiteFilterDuotoneThreshold, _LiteFilterFilm,
...
_LiteFilterOutlineWidth, _LiteFilterOutlineTolerance, _LiteFilterAstral,
...
_LiteFilterTrippy, _LiteFilterTrippyPower, _LiteFilterTrippySpread, 
_LiteFilterTrippySpeed;
...
#endif // _KEYWORD_ENABLE_LITE_FILTER
#ifdef _KEYWORD_ENABLE_LITE_OVERLAY
float _LiteOverlaySizeX, _LiteOverlaySizeY, _LiteOverlayOffsetX,
_LiteOverlayOffsetY, _LiteOverlayTransparency, _LiteOverlayAnimated,
_LiteOverlayFramesX, _LiteOverlayFramesY, _LiteOverlayFrames,
_LiteOverlaySpeed, _LiteOverlayScrub, _LiteOverlayBlendMode, _LiteOverlayVR,
_LiteOverlayVRPreview, _LiteOverlayVRSizeX, _LiteOverlayVRSizeY,
_LiteOverlayVROffsetX, _LiteOverlayVROffsetY;
sampler2D _LiteOverlayTexture;
#endif // _KEYWORD_ENABLE_LITE_OVERLAY
#ifdef _KEYWORD_ENABLE_LITE_GLITCH
float _LiteGlitchAmount, _LiteGlitchScale, _LiteGlitchPower,
    _LiteGlitchUVs, _LiteGlitchChromatic;
#endif // _KEYWORD_ENABLE_LITE_GLITCH

//|===============================================|
//|				 pixel kernel 					  |
//|===============================================|
float4 pixel(v2f i) : SV_Target : COLOR {
	// preparing the scene
	#if UNITY_SINGLE_PASS_STEREO
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i)
	#endif
	float2 sceneUVs = float2(i.grabPos.xy / i.grabPos.w);
	float4 sceneColors = tex2D(_JLPass, sceneUVs);
	if (i.precalculations.x >= _LiteRenderingFalloffEnd) {
        ...
        #endif // _KEYWORD_ENABLE_LITE_UV_MANIPULATION

        #ifdef _KEYWORD_ENABLE_LITE_GLITCH
        effectGlitchUVs(sceneUVs, _LiteGlitchScale, _LiteGlitchAmount, _LiteGlitchUVs);
        #endif // _KEYWORD_ENABLE_LITE_GLITCH
        ...
        // building the pixels
        sceneColors = tex2D(_JLPass, sceneUVs);
        float4 cleanColors = sceneColors;

        //float test_val = makeSimpleGlitchUVs(sceneUVs, 3.43, 0.272, 1, 20000);
        //return float4(test_val, 0, 0, 1);

        #ifdef _KEYWORD_ENABLE_LITE_GLITCH
        effectGlitchChromatic(sceneColors, sceneUVs, _LiteGlitchScale, _LiteGlitchAmount, _LiteGlitchChromatic, _JLPass);
        #endif // _KEYWORD_ENABLE_LITE_GLITCH

        // blur 
        #ifdef _KEYWORD_ENABLE_LITE_BLUR
        effectBlur(sceneColors, sceneUVs, _LiteBlurStyle, _LiteBlurRadius, _LiteBlurPower, _LiteBlurTransparency, _LiteBlurColor, _JLPass, i.zoom, _LiteBlurDithering);
        #endif // _KEYWORD_ENABLE_LITE_BLUR

        // coloring
        #ifdef _KEYWORD_ENABLE_LITE_COLORING
        effectColoring(sceneColors, _LiteColoringRGBMultiply, _LiteColoringRGBOverlay, _LiteColoringRGBOverlayTransparency, _LiteColoringHSVStyle, _LiteColoringHSVh, _LiteColoringHSVs, _LiteColoringHSVv, _LiteColoringInvert, _LiteColoringDrain, _LiteColoringDarkness, _LiteColoringBrightness, _LiteColoringEmission, _LiteColoringPosterization, _LiteColoringColorGrading, _LiteColoringColorGradingTone, _LiteColoringSharpness, sceneUVs, _JLPass, _LiteColoringRainbow, _LiteColoringRainbowSpeed);
        #endif // _KEYWORD_ENABLE_LITE_COLORING
        ...
        // outline
        effectFilterOutline(sceneColors, _LiteFilterOutline, _LiteFilterOutlineWidth, _LiteFilterOutlineTolerance, _LiteFilterOutlineColor, sceneUVs, _JLPass);
        // astral
        effectFilterAstral(sceneColors, _LiteFilterAstral, _LiteFilterAstralZoom, _LiteFilterAstralTransparency, _LiteFilterAstralColor, i.zoom, sceneUVs, _JLPass);
        // neon
        effectFilterNeon(sceneColors, _LiteFilterNeon, _LiteFilterNeonWidth, _LiteFilterNeonTransparency, _LiteFilterNeonHue, sceneUVs, _JLPass);
        // trippy
        effectFilterTrippy(sceneColors, _LiteFilterTrippy, _LiteFilterTrippyPower, _LiteFilterTrippySpread, _LiteFilterTrippySpeed, sceneUVs);
        #endif // _KEYWORD_ENABLE_LITE_FILTER

        // fog
        #ifdef _KEYWORD_ENABLE_LITE_FOG
        effectFog(sceneColors, sceneUVs, _LiteFogDensity, _LiteFogDistribution, _LiteFogColor, _LiteFogSafespace, _LiteFogSafespaceSize, i.grabPos, i.worldDir, i.worldPos, i.pos, i.rayCast);
        #endif // _KEYWORD_ENABLE_LITE_FOG

        // overlay
        #ifdef _KEYWORD_ENABLE_LITE_OVERLAY
        effectOverlay(sceneColors, _LiteOverlaySizeX, _LiteOverlaySizeY, _LiteOverlayOffsetX, _LiteOverlayOffsetY, _LiteOverlayTransparency, _LiteOverlayTexture, sceneUVs, i.overlayCoordinates.xyz, _LiteOverlayAnimated, _LiteOverlayFramesX, _LiteOverlayFramesY, _LiteOverlayFrames, _LiteOverlaySpeed, _LiteOverlayScrub, _LiteOverlayBlendMode, _LiteOverlayVR, _LiteOverlayVRPreview, _LiteOverlayVRSizeX, _LiteOverlayVRSizeY, _LiteOverlayVROffsetX, _LiteOverlayVROffsetY);
        #endif // _KEYWORD_ENABLE_LITE_OVERLAY
        ...
        // returning
            ...
            float returnMix = i.precalculations.y * getAudioLink();
        #else // _KEYWORD_ENABLE_LITE_AUDIO_LINK
            float returnMix = i.precalculations.y;
        #endif // _KEYWORD_ENABLE_LITE_AUDIO_LINK
        sceneColors.rgb = lerp(cleanColors, sceneColors.rgb, returnMix);
        return sceneColors;
    }
