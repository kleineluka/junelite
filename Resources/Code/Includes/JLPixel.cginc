...
void effectGlitchUVs(inout float2 inputUVs,
    ...
    [branch] if (inputPower != 0) {
        float offset = makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 1, 20000);
...
void effectGlitchChromatic(inout float4 inputColors,
    ...
    sampler2D inputPass) {
    [branch] if (inputPower != 0) {
        float offsetR = makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 2.1, 20000);
        float offsetB = makeSimpleGlitchUVs(inputUVs, inputScale, inputAmount, 4.5, 20000);
...
_LiteOverlaySpeed, _LiteOverlayScrub;
...
float4 pixel(v2f i) : SV_Target : COLOR {
	...
	if (i.precalculations.x >= _LiteRenderingFalloffEnd) {
        ...
        // blur 
        #ifdef _KEYWORD_ENABLE_LITE_BLUR
        effectBlur(sceneColors, sceneUVs, _LiteBlurStyle, _LiteBlurRadius, _LiteBlurPower, _LiteBlurTransparency, _LiteBlurColor, _JLPass, i.zoom);
        #endif // _KEYWORD_ENABLE_LITE_BLUR
        ...
        // overlay
        #ifdef _KEYWORD_ENABLE_LITE_OVERLAY
        effectOverlay(sceneColors, _LiteOverlaySizeX, _LiteOverlaySizeY, _LiteOverlayOffsetX, _LiteOverlayOffsetY, _LiteOverlayTransparency, _LiteOverlayTexture, sceneUVs, i.overlayCoordinates.xyz, _LiteOverlayAnimated, _LiteOverlayFramesX, _LiteOverlayFramesY, _LiteOverlayFrames, _LiteOverlaySpeed, _LiteOverlayScrub);
        #endif // _KEYWORD_ENABLE_LITE_OVERLAY
